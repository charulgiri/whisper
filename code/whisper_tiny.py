...
model = AutoModelForSpeechSeq2Seq.from_pretrained(output_dir)
...
nb_16k = DatasetDict()
nb_16k["train"] = load_dataset("NbAiLab/NPSC", '16K_mp3_bokmaal', split="train").select(range(1000))
nb_16k["validate"] = load_dataset("NbAiLab/NPSC", '16K_mp3_bokmaal', split="validation").select(range(300))
...
checkpoint_dir = "/home/coder/whisper-tiny_nob/checkpoint-400"
training_args = Seq2SeqTrainingArguments(
    output_dir="./whisper-tiny_nob_2",  # change to a repo name of your choice
...
# print(configuration)
training =  trainer.train(resume_from_checkpoint=checkpoint_dir)
evalu =  trainer.evaluate(nb_16k["test"])
print(evalu)
trainer.save_model("model_tiny2")
feature_extractor.save_pretrained("whisper_feature_extractor2")
processor.save_pretrained("whisper_processor2")